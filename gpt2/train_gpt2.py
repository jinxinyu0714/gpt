...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # y = att @ v
        # this is using falsh attention to replace original attention
...
class DataLoaderLite:
    def __init__(self, B, T, process_rank, num_processes):
        ...
        #print(f'1 epoch = {len(self.tokens) // (B * T)}m batches')
...
# CUDA_VISIBLE_DEVICES=0,1 torchrun --standalone --nproc_per_node=2 train_gpt2.py
...
if ddp:
    #use of DDP atm demands CUDA, we set the device appropriately according to rand
    assert torch.cuda.is_available(), "for now need CUDA for DDP"
    ...
    master_process = ddp_rank == 0 # this porcess will do logging, checkpointing etc.
else:
    # venilla, non-DDP run
    ddp_rank = 0
    ddp_local_rank = 0
...
def get_lr(it):
    ...
    # 2) fi it > lr_dacay_iters, return min learning rate
    if it > max_steps:
        return min_lr
...
# optimize
#opitmizer = torch.optim.Adam(model.parameters(), lr=3e-4, betas=(0.9, 0.95), eps=1e-8)
opitmizer = raw_model.configure_optimizers(weight_decay=0.1, learning_rate=6e-4, device=device)
for step in range(max_steps):
    ...
    lr = get_lr(step)
    for param_group in opitmizer.param_groups:
        param_group['lr'] = lr
    opitmizer.step()
    torch.cuda.synchronize()
    t1 = time.time()
    dt = (t1 - t0)
    tokens_processde = train_loader.B * train_loader.T * grad_accum_steps * ddp_world_size
    tokens_per_sec = tokens_processde / dt
    if master_process:
